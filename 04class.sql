...
ORDER BY
	2 DESC,
	1; 
--For product: Intel SSDPECME040T401
...
SELECT
	emp.last_name
	--,LEN(emp.last_name) -- Oracle length
...
--' last name'
--'last name'
--как искать например в 8-м символе определенное значение?
select SUBSTRING('ry34567890', 8, 1) 
/*
 * +--------+
 * |  INNER |
 * +--------+
...
select
	*
	--p.description , pc.category_name 
from
	db_laba.dbo.products p
...
SELECT
	...
	--emp1.employee_id , emp1.manager_id ,
	--'managed by ',
	emp2.first_name + ' ' + emp2.last_name as manager ,
	--emp2.employee_id , emp2.manager_id ,
	COALESCE(emp2.first_name + ' ' + emp2.last_name, 'Tommy Bailey') as manager_2,
	COALESCE(emp2.first_name + ' ' + emp2.last_name, emp1.first_name + ' ' + emp1.last_name) as manager_3
from
	db_laba.dbo.employees emp1
